...
<template>
    <div>
        ...
        <video :id="videoId" style="position: absolute; top: 10px; right: 10px; width: 200px;" :src="videoSrc" muted autoplay loop ref="videoRef1"
            crossorigin="anonymous" @timeupdate="handleTimeUpdate" @loadedmetadata="handleLoadedMetadata"
            @ended="handleVideoEnded">
        </video>
        <img src="./321123.png" style="display: none; " ref="imgRef" alt="">
    </div>
</template>
...
const addRectangle = () => {
    ...
    //         transparent: true,
    //         // 关键：通过纹理矩阵让视频跟随多边形旋转
    //         textureMatrix: Cesium.Matrix3.fromRotation(props.heading)
    //     });
    // };
    ...
    rectangleEntity = viewer.value.entities.add({
        ...
        polygon: {
            ...
            // material: new Cesium.CallbackProperty(getVideoMaterial, false),
...
onMounted(() => {
    ...
    videoRef1.value.currentTime = props.currentTime
    videoRef.value?.play();
    setTimeout(() => {
