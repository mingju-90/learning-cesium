...
const createPolygon = () => {
    const polygonEntity = entityCollection.entities.add({
        polygon: {
            hierarchy: new Cesium.CallbackProperty(() => {
                if (!mousePosition.value) return { positions: [] }
                const coordinates = [...polygonPosition.value, mousePosition.value]
                return {
                    positions: coordinates.map(({ longitude, latitude }) => Cesium.Cartesian3.fromDegrees(longitude, latitude))
                }
            }, false),
